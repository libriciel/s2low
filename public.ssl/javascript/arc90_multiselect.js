...
a$.NO_SELECTION = 'Pas de sélection';
a$.SELECTED     = 'Options selected';
a$.SELECT_ALL   = 'tout Selectioner';
a$.SelectAllMin = 6;
...
//  or if a$.WhenToUse is set to class or multiple
    ...
    for (var i = 0, l = 1; i < l; i++) { //S.length
        ...
        if (s != null && ((a$.WhenToUse == 'class' &&  s.className.indexOf('arc90_multiselect') >= 0) || (a$.WhenToUse == 'multiple' && s.multiple) || (a$.WhenToUse == 'all'))) {
            ...
            a$.newEvent(div, 'mouseout', function (event) {
                a$.msBodyTimer = setTimeout('a$.closeSelect("' + id + '")', 1500); });
            a$.newEvent(div, 'mouseover', function (event) {
                clearTimeout(a$.msBodyTimer); a$.msBodyTimer = null; });
            a$.newEvent(expbody, 'mouseout', function (event) {
                a$.msBodyTimer = setTimeout('a$.closeSelect("' + id + '")', 1500); });
            a$.newEvent(expbody, 'mouseover', function (event) {
                clearTimeout(a$.msBodyTimer); a$.msBodyTimer = null; });
            a$.newEvent(ul, 'mouseout', function (event) {
                clearTimeout(a$.msBodyTimer); a$.msBodyTimer = null; });
            a$.newEvent(ul, 'mouseover', function (event) {
                clearTimeout(a$.msBodyTimer); a$.msBodyTimer = null; });
            ...
            for (var j = 0; j < m; j++) {
                ...
                a$.newEvent(chkbx, 'click', function (event) {
                    ...
                    if (allbx) {
                        a$.chk(allbx, (allbx.checked = false));
                    }
                });
                ...
                if (a$.isIE) {
                    ...
                    a$.newEvent(label, 'click', function (event) {
                        a$.cancelbubble(event); }); // cancel so li event doesn't get activated
                }
...
a$.multiSelect = function (chk, value, parent) {
    ...
    // modified by TAN Hao.
    //to is the input tag where we transfer the data.
    //what he has done have a serious proble, if i select sevral time, he will give a serval selection duplicate.
    //and his code to delete duplicate do not works:
    /*
    ...
    */
    ...
    to.value = vals;
    vals = c > 1 ? vals.slice(0, vals.length - 1) : v;
    title.innerHTML = vals;
    t = a$.e(pid).title;
...
a$.expcol = function (parent) {
    ...
    for (var i = 0, l = x.length; i < l; i++) {
        if (x[i].className.indexOf("-click") >= 0) {
            x[i].onclick = a$.expcolclick;
        }
    }
}
