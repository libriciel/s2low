...
//      forceIframeTransport: true,
        ...
        submit: function (e, data) {
            ...
            if (verif.intitule && verif.prefix && verif.noErrorFiles) {
                ...
                var msg = '';
                if (!verif.intitule) {
                    msg += 'Le champ "Intitul� du lot" est vide.';
                }
                if (!verif.prefix) {
                    msg += '\nLe champ "Pr�fixe des num�ros internes" est vide.';
                }
        ...
        always: function (e, data) {
            ...
            //on masque la liste des fichiers et on affiche une nouvelle liste (contournement du bug d affichage des fichiers envoy�s si on en supprime un de la liste avant l envoi)

            if (lvl != 1) {
                var labelfilelist = $('<span>Liste des fichiers envoy�s : </span>');
                var filelist = $('<ul></ul>');
                jQuery.each(obj, function (key, value) {
                    var isfile = false;
                    jQuery.each(value, function (key2, value2) {
