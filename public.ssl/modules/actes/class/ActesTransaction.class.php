...
class ActesTransaction extends DataObject
{
    ...
    public array $files = [];
    protected string $objectName = 'actes_transactions';
   ...
   */
    ...
    public static function getTypeReponse($transactionType, $reponseType): bool|string
    {

        $typeReponse = [
        3   => [4 => 'Transmission de pièces complémentaires',
                    3 => "Refus explicite d'envoi de pièces complémentaires"
        ],
        4 => [4 => "Lettre de justification de l'acte",
     ...
     * @throws \Exception
     */
    public static function getTransactionNaturesIdDescr(): array
    {
  ...
  */
   ...
   */
    public function getFluxRetour($status_id): bool|string
    {
     ...
     * @throws \Exception
     */
    public function getCurrentMesssage(): false|string
    {
        if (! $this->id) {
            return false;
        }
     ...
     * @throws \Exception
     */
    public function setNotification($emails, $send_sources): bool
    {
   ...
   */
    public function getStdFileName($env, $use_serial = true, $code_pj = ''): string
    {
        if ($this->type == 6) {
     ...
     * @throws \Exception
     */
    public static function getTransactionNatureDescr($id): bool|array
    {
        $id = intval($id);
        if (!empty($id)) {
   ...
   * \return True en cas de succès, false sinon
   */
    public function addActeSign($sign, $readFile = true): bool
    {
   ...
   * \return True en cas de succès, false sinon
   */
    public function addSign($type, $sign, $readFile): bool
    {
   ...
   */
    public function storeSign($type, $sign): bool
    {
        if ($type == 'acte') {
   ...
   * \return True en cas de succès, false sinon
   */
    public function addAttachmentSign($sign, $readFile = true): bool
    {
   ...
   * \return True en cas de succès, false sinon
   */
    public function generateMessageXMLFile($xml_name): bool
    {
        switch ($this->type) {
            case '1':
   ...
   */
    public function generateActeXMLFile($xml_name): string
    {
        $xml_name .= '_0.xml';
      ...
      // Les codes matières
        for ($i = 1; $i <= 5; $i++) {
            $var = 'classif' . $i;
            if (!empty($this-> $var) && is_numeric($this-> $var)) {
    ...
    public function generateReponseCourrierXMLFile($xml_name): bool|string|null
    {
        $xml_name .= '_0.xml';
        $this->xmlFileName = $xml_name;
   ...
   * \return Le XML généré ou false en cas d'échec
   */
    public function generateCancelXMLFile($xml_name): ?string
    {
        $xml_name .= '_0.xml';
   ...
   * \return Le XML généré ou false en cas d'échec
   */
    public function generateClassifRequestXMLFile($xml_name): string
    {
        $xml_name .= '_0.xml';
     ...
     * @throws \Exception
     */
    public function createFromXML($xmlFile, ActesClassificationCodesSQL $actesClassificationCodesSQL): bool
    {
      ...
      // Détermination du type de transaction
        switch (@ dom_import_simplexml($this->xmlObj)->nodeName) {
            ...
            case 'actes:PieceComplementaire':
                $rep = $this->setDataFromCourrier(3, $xmlFile);
                break;
            ...
            case 'actes:ReponseLettreObservations':
                $rep = $this->setDataFromCourrier(4, $xmlFile);
                break;
   ...
   * \return True en cas de succès, false sinon
   */
    public function addActeFile($name, $dest_name, $path = null, $validate = true, $code_pj = ''): bool
    {
   ...
   * \return True en cas de succès, false sinon
   */
    public function addFile($type, $name, $dest_name, $path = false, $validate = true, $code_pj = ''): bool
    {
      ...
      // c'est que le fichier se trouve déjà dans son emplacement définitif (dans le cas d'import de .tar.gz)
        ...
        if ($validate && $path) {
            ...
            if ($type == 'acte' && $this->type == 1) {
                ...
                if ((isset($this->files['acte']['mimetype']) && $this->files['acte']['mimetype'] == 'application/xml') && ($ext != 'pdf')) {
        ...
        if (!$import) {
            if ($path) {
                if (!Helpers :: createDirTree(dirname($this->rootDir . '/' . $new_name))) {
                    ...
                    return false;
                } elseif (!copy($path, $this->rootDir . '/' . $new_name)) {
   ...
   * \return True en cas de succès, false sinon
   */
    public function addAttachmentFile($name, $dest_name, $path = null, $validate = true, $code_pj = ''): bool
    {
    ...
    private function setDataFromCourrier($type, $xmlFile, $isRefus = false): bool
    {
  ...
  */
   ...
   * \return True en cas de succès, false sinon
  */
    public function purgeFiles(): bool
    {
     ...
     * @throws \Exception
     */
    public function fetchWorkflow(): bool|array
    {
  ...
  //Message 2, 3, 4 et 5
     ...
     * @throws \Exception
     */
    public function hasPendingCancelTrans(): bool
    {
        if (isset($this->id)) {
  ...
  /**********************/
     ...
     * @throws \Exception
     */
    public function save($validate = true, $bouchon_4_strict_standard = true): bool
    {
        $new = false;
        if ($this->isNew()) {
            $new = true;
     ...
     * @throws \Exception
     */
    public function setNewStatus($new_status_id, $message): bool
    {
     ...
     * @throws \Exception
     */
    public function getCourrierInfo(): bool|array
    {
        $renvoie = [];
        ...
        while ($row = $result->get_next_row()) {
            if ($row['related_transaction_id'] == '') {
                $renvoie[$row['id']] = ['type' => $row['type'],
                                    'sens' => 'reçu'];
            } else {
                $renvoie[$row['related_transaction_id']] = ['type' => $row['type'],
                                    'sens' => 'envoye'];
            }
        }
        return $renvoie;
    ...
    public function setEnAttente($en_attente): void
    {
        $this->en_attente = $en_attente;
    }

    public function setEnAttenteDeSignature($is_en_attente_de_signature): void
    {
