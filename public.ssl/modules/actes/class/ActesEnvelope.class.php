...
<?php
/**
 * \class ActesEnvelope ActesEnvelope.class.php
 * \brief Cette classe permet de gérer les enveloppes ACTES
 * \author Jérôme Schell <j.schell@alternancesoft.com>
 * \date 27.07.2006
 * 
 *
 * Cette classe fournit des méthodes de gestion des enveloppes
 * Actes
 *
 * Modifications :
...
class ActesEnvelope extends DataObject {
  ...
  */
   ...
   */
  public function generateEnvelopeXMLFile() {
	if (! $serial = $this->getNextEnvelopeSerial()) {
	  return false;
	}
    ...
    $xml .= "  <actes:Referent>\n";
    $xml .= "   <actes:Nom>" . Helpers::escapeForXML($this->name) . "</actes:Nom>\n";
	...
	if (strlen($this->return_mail) > 0) {
	  ...
	  $this->errorMsg = "Informations manquantes pour générer l'enveloppe";
	  return false;
	}
  ...
  */
  public function getNextEnvelopeSerial($authority = null) {
	if ($authority || isset($this->user_id)) {
	  if (! $authority) {
		$user = new User($this->user_id);
		$user->init();
		$authority = new Authority($user->get("authority_id"));
	  }
	  ...
	  $sql = "SELECT reset_date, serial FROM actes_envelope_serials WHERE authority_id=" . $authority->getId() . " FOR UPDATE";
	  ...
	  if (! $result->isError()) {
		if ($result->num_row() <= 0) {
		  ...
		  $date = date('Y-m-d');
		  $sql = "INSERT INTO actes_envelope_serials (authority_id, reset_date, serial) VALUES(" . $authority->getId() . ", '" . $date . "', 2)";
		  ...
		  if (strcmp($today, $date) != 0) {
			...
			$sql = "UPDATE actes_envelope_serials SET reset_date='" . $today . "', serial=2 WHERE authority_id=" . $authority->getId();
		  ...
		  // pour le bug 240, modifié par HTan, 15-12-2008
			...
			$sql = "UPDATE actes_envelope_serials SET serial=serial+1 WHERE authority_id=" . $authority->getId();
  ...
  */
  ...
  public function getEnvelopesList($cond = "") {
	if (! $this->pagerInit('DISTINCT actes_envelopes.id, actes_envelopes.user_id, actes_envelopes.siren, ' .
