<?php
/*
 * T��IS - Copyright 2006 Alternance-Soft
 ...
 * 
 *
 * Cette page affiche les d�ails d'une transaction Actes et 
...
if (! $me->authenticate()) {
  $_SESSION["error"] = "�hec de l'authentification";
...
if (isset($id) && ! empty($id)) {
  ...
  $_SESSION["error"] = "Pas d'identifiant de transaction sp�cifi�";
  header("Location: " . WEBSITE_SSL . "/modules/helios/index.php");
  exit();
}
...
$html .= "<h3>R�cuperation du fichier post� ";
$html .= "<a href=\"" .WEBSITE_SSL. "/modules/helios/helios_download_file.php?id=" .$id. "\" title=\"T�l�charger le fichier\">".$trans->getFilenameForID($id)."</a> </h3>";
  ...
  foreach ($workflow as $stage) {
	...
	$html .= "  <td>" . $status[$stage["status_id"]] ;
  	if($stage["status_id"] == 4 || $stage["status_id"] == 6 )
       $html .= " <a href=\"" .WEBSITE_SSL. "/modules/helios/helios_download_acquit.php?id=" .$id. "\" title=\"T�l�charger l'acquittement\">voir</a> </h3>";
    $html .= "</td>\n";
