 ...
 *Etat Civil et permet de les modifier ou d'en cre�r de nouvelles
 *
 * Modifications :
...
function afficheWarning(){
  ...
  var msg = "Voulez-vous vraiment affecter les " + n + " transactions s�ectionn�s ?\\n";
  msg += "Cette action est non r�ersible et est sous votre enti�e responsabilit�";
  return confirm(msg);
}
//]]>
</script>
...
//deja etat_civil!!!!
$html = "<div id=\"content\">\n";
$html .= "<h1>D�mat�rialisation de documents d'�tat civil</h1>\n";
...
//$fnum cred ca vine de la "num�o contient"
if (strlen($fnum) > 0) {
  $html .= " value=\"" . $fnum . "\"";
}
...
//colectivit�!!  Ptr superuser!!!!
$colspan = 4;
if ($me->isSuper()) {
  $html .= "<td class=\"title\">Collectivit�nbsp;:</td>\n";
...
$html .= "<td colspan=\"" . $colspan . "\"><a href=\"" . WEBSITE_SSL . "/modules/etat_civil/index.php\" class=\"bouton\">Remise&nbsp;�&nbsp;z�ro</a></td>\n";
$html .= "</tr>\n";
...
if (!$me->isSuper() && $me->canEdit($module->get('name'))) {
  ...
  if ($module->getParam("paper") == "on") {
    $html .= "<p>Le syst�e est actuellement en mode &nbsp;papier&nbsp;. Dans ce mode il est impossible de cr�r de nouvelle transaction. Les transferts doivent se faire par les moyens classiques (non d�at�ialis�).</p>\n";
  } else {
  //  $html .= "<a href=\"" . WEBSITE_SSL . "/modules/actes/actes_transac_add.php\" class=\"bouton\">Cr�r une transaction</a>\n";
...
$filter = array ();
// Construction cha�e de filtrage
if ($me->isSuper()) { // Le super utilisateur voit toutes les collectivit�
 //verifica $authority
...
elseif ($me->isAdmin()) { // Un admin d'une collectivit�ne voit que les transactions de sa collectivit�  $filter[] .= "users.authority_id='" . $me->get("authority_id") . "'";
} else {
...
$html .= "<h2>Liste des fichiers post�s</h2>\n";
...
if (count($envelopes) > 0) {
  ...
  /*
  $html .= "<a href=\"#tedetis\" onclick=\"javascript:show_all();\" title=\"D�lier toutes les enveloppes\">[&nbsp;Tout d�lier&nbsp;]</a>\n";
...
/*
    ...
    $html .= "<a href=\"" . Helpers :: getURLWithParam(array (
      "order" => "id",
      "sortway" => $sortWay
    )) . "\" title=\"Trier par identifiant\">" . $envelope["id"] . "</a> d�os� le ";
    $html .= "<a href=\"" . Helpers :: getURLWithParam(array (
      "order" => "date",
      "sortway" => $sortWay
 //   )) . "\" title=\"Trier par date de d��\">" . Helpers :: getDateFromBDDDate($envelope["submission_date"], true) . "</a> contenant " . count($transactions);
    )) . "\" title=\"Trier par date de d��\">" . Helpers :: getDateFromBDDDate(etat_civilTransactionWorkflow::getDatePoste($envelope["id"]), true) . "</a>";
 */
...
$transaction_id=$envelope["id"];
      ...
      $html .= " <td><a href=\"" . WEBSITE_SSL . "/modules/etat_civil/etat_civil_transac_show.php?id=" .$envelope["id"]. "\" class=\"icon\"><img src=\"" . WEBSITE_SSL . "/custom/images/erreur.png\" alt=\"image_modif\" title=\"Afficher le d�ail\" /></a></td>\n";
      $html .= "</tr>\n";
 ...
 // $html .= "</dl>\n";
/*
  $html .= "<a href=\"#tedetis\" onclick=\"GereChkbox('div_chck','1');\" title=\"Tout s�ectionner\">[&nbsp;Tout s�ectionner&nbsp;]</a>\n";
  $html .= "<a href=\"#tedetis\" onclick=\"GereChkbox('div_chck','0');\" title=\"Tout d��ectionner\">[&nbsp;Tout des�ectionner&nbsp;]</a>\n";
  $html .= "<a href=\"#tedetis\" onclick=\"GereChkbox('div_chck','2');\" title=\"Inverser la s�ection\">[&nbsp;Inverser la s�ection&nbsp;]</a>\n";

  // Cloture des transaction s�ectionn�s
  $html .= "<div class=\"action\">\n";
  $html .= "Passer les transactions s�ectionn�s en �at&nbsp;<select name=\"status\"><option value=\"valid\">Valid�/option><option value=\"invalid\">Refus�/option></select>";
  $html .= "<input type=\"submit\" class=\"submit_button\" value=\"Ex�uter\"/>\n";
  $html .= "</div>\n</form>";
*/
} else {
...
/*
  ...
  $html .= "Pas de transaction trouv� correspondant aux crit�es de filtrage.";
}
$html .= "</div>\n";
*/
...
/*
    ...
    foreach ($transactions as $transaction) {
      ...
      $html .= " <td>" . $status[$transaction->getCurrentStatus()] . "</td>\n";
      //$html .= " <td>" . $transaction->get("unique_id") . "</td>\n";
      $html .= " <td><a href=\"" . WEBSITE_SSL . "/modules/actes/actes_transac_show.php?id=" . $transaction->getId() . "\" class=\"icon\"><img src=\"" . WEBSITE_SSL . "/custom/images/erreur.png\" alt=\"image_modif\" title=\"Afficher le d�ail\" /></a></td>\n";
      $html .= "</tr>\n";
    }
