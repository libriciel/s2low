...
/**
 * \bref list les email re�u.
 * \bref appel� just par mailctroller::run();
 ...
 */
	protected function executeList() {
     ...
     //FIXME : ca n'a rien � foutre l�: faire un script interm�diaire
     if ($deleteId!=null)
     {
   ...
   * FIXME FIXME
   * 
   * passer par un script interm�diaire
   * 
   * FIXME FIXME
   * 
   * 
 * \bref cr�er un nouvel email.
 * \bref appel� just par mailctroller::run();
 * \param: pas de parametre
 */
  ...
  /**
 * \bref afficher le d�tail d'un email.
 * \bref appel� juste par mailController::run();
 * \param: pas de param�tre
 */
  ...
  //fini de la tratement
   ...
   include dirname(__FILENAME__)."/template/show.php";
	...
	//HACK b�quille pour transformer les mails ...
 ...
 */
  protected function executeSend()
  {
    ...
    if ($mailTo==null)
    {
    	$returnMsg="Vous �tes oblig� d'avoir un destinataire.";
    	include dirname(__FILENAME__)."/template/sendfailed.php";	
    	return false;
    }
    else
    ...
    if ($subject==null)
    {
    	$returnMsg= "Vous �tes oblig� d'avoir un objet.";
      include dirname(__FILENAME__)."/template/sendfailed.php";	
    	return false;
    }
    if ($message==null)
    {
    	$returnMsg= "Le champ message doit �tre rempli";
    	include dirname(__FILENAME__)."/template/sendfailed.php";	
    	return false;
    }
    if ($mailCC)
    ...
    //-------fini de la v�rification
    //----ini mail tranaction.
    // mail transaction faut absolutment inite avant tous les autre op�ration car tous les autre tableau need 
    // mail transaction id.
    $mailTransaction=new mail_transaction();
    ...
    //--------------------------------------------------------------------  
    //FileNumber = le nombre de File est attach�. Il commence par 1. 
    //Il est d�fini dans le fichier de javascript file: mail.js
    $InputFileName=array();
    ...
    if ($FileNumber !=null)
    {
       ...
       for ($i = 1; $i <= $FileNumber; $i++)
       {
          ...
          // le nom de chaque file =uploadFile1, uploadFile2,,,,jusqu'� FileNumber
          // parcque des fois les utilisateur supprime une fichier qu'il a d�jas ajout� et le FileNumber va pas diminuer enmeme temp
	...
	if ($myAuthority->get('email_mail_securise')){
  	...
  	if (count($InputFileName)>0)
  	{
        ...
        // cr�er un repertoir de md5
  		$newdir=MAIL_FILES_UPLOAD_ROOT.$mailTransaction->getFNDownload().'/';
  		if (!mkdir ($newdir, 0755, true))
  		{
  			$returnMsg="La cr�ation de r�pertoire a echou�.";
	    ...
	    foreach ($InputFileName as $Filename)
	    {
             ...
             if ($temp->newSave($Filename,$Transaction_id,$newdir))
 		     ...
 		     {
	 		     	$returnMsg="Le chargement du fichier sur le server a �chou� : " . $temp->getLastError();
		 		    if (!Log :: newEntry(LOG_ISSUER_NAME, $returnMsg, 3, false, 'USER', $module->get("name"), $me))
					  {
					    $returnMsg= "\nErreur de journalisation.";
					    include dirname(__FILENAME__)."/template/sendfailed.php";	
	...
	{	
	  	$returnMsg= "�chec lors de l'envoi.";
			if (!Log :: newEntry(LOG_ISSUER_NAME, $returnMsg, 3, false, 'USER', $module->get("name"), $me))
		  {
		    $returnMsg= "\nErreur de journalisation.";
		    include dirname(__FILENAME__)."/template/sendfailed.php";	
...
/**
 * \bref envoyer ajouter ou supprimer un contact dans l'annuaire.appel� juste par mailController::run();
 *		
 * \param pas de param�tre
 */
  protected function executeAnnuaire()
  {
  	...
  	if ($idArray != null)
  	{
  		...
  		unset($groupe_id);
  		$_SESSION['last_message'] = "Op�ration effectu� avec succ�s";
  	}
 ...
 * \bref		r�cup�rer les nouveau email
 * \bref		trouver le quelle mail n'est pas r�ussi d'envoyeer
 * \bref		sauvegarder dans la base de donnee
 * \bref	appel� just par mailctroller::show();
 * \param: pas de parametre
 */
  ...
  protected function SaveMailEmis($mail, $Transaction_id,$type)
  {
	if ($mail==null)
     ...
     {	
        ...
        //v�rifier le mail adress exist d�jas ou pas
        //si non; met dans MailAnnuaireArray pour traiter ap�s.
