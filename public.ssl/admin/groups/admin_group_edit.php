...
$html .= "<p id=\"back-transaction-btn\"><a href=\"" . \S2lowLegacy\Class\LegacyObjectsManager::getLegacyObjectInstancier()->get(\S2low\Helpers\RequeteHelper::class)->getLink("/admin/groups/admin_groups.php\" class=\"btn btn-default\">Retour liste groupes</a></p>\n");

$html .= "<h2>" . $modStr . " groupe de collectivités</h2>\n";
$html .= "<form class=\"form form-horizontal\" action=\"" . \S2lowLegacy\Class\LegacyObjectsManager::getLegacyObjectInstancier()->get(\S2low\Helpers\RequeteHelper::class)->getLink("/admin/groups/admin_group_edit_handler.php\" method=\"post\" name=\"form\" enctype=\"multipart/form-data\" onsubmit=\"javascript:return validateForm(" . $group->getValidationTrio('name', 'status') . ")\">\n");
...
if ($mod && $group->isEmpty($group->getId())) {
    $html .= "<form action=\"" . \S2lowLegacy\Class\LegacyObjectsManager::getLegacyObjectInstancier()->get(\S2low\Helpers\RequeteHelper::class)->getLink("/admin/groups/admin_group_delete.php\" onsubmit=\"return confirm('Voulez-vous vraiment supprimer définitivement ce groupe ?')\" method=\"post\">\n");
    $html .= "<input type=\"hidden\" name=\"id\" value=\"" . $group->getId() . "\" />\n";
...
if ($mod) {
      $html .= "<form class=\"form form-horizontal\" action=\"" . \S2lowLegacy\Class\LegacyObjectsManager::getLegacyObjectInstancier()->get(\S2low\Helpers\RequeteHelper::class)->getLink("/admin/groups/add-siren-controler.php\" method=\"post\">\n");
      $html .= "<input type=\"hidden\" name=\"id\" value=\"" . $group->getId() . "\" />\n";
      $html .= "<div class=\"form-group\">\n";
