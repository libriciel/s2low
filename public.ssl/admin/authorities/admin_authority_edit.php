...
if ($me->isGroupAdminOrSuper()) {
    $html .= "<p id=\"back-transaction-btn\"><a href=\"" . \S2lowLegacy\Class\Helpers\UrlHelper::getLink("/admin/authorities/admin_authorities.php\" class=\"btn btn-default\">Retour liste collectivités</a></p>\n");
}
...
$html .= " </div>\n";
$html .= "<script src=\"" . \S2lowLegacy\Class\Helpers\UrlHelper::getLink('/jsmodules/handleSirenGroupe.js') . "\"></script>";
...
if ($me->isGroupAdminOrSuper()) {
    ...
    $html .= "  <a class=\"link_white\" href=\"#tedetis\" onclick=\"javascript:window.open('" . \S2lowLegacy\Class\Helpers\UrlHelper::getLink("/common/select_popup.php?type=authority_type', 'Selectattribut', 'location=no,scrollbars=yes,menubar=no,status=no,toolbar=no,directories=no,width=512,height=560');\" id=\"authority_type_text\">");
...
if ($me->isGroupAdminOrSuper()) {
    $html .= "  <a class=\"link_white\"href=\"#tedetis\"  class=\"form-control\" onclick=\"javascript:window.open('" . \S2lowLegacy\Class\Helpers\UrlHelper::getLink("/common/select_popup.php?type=department', 'Selectattribut', 'location=no,scrollbars=yes,menubar=no,status=no,toolbar=no,directories=no,width=300,height=560');\" id=\"department_text\">");
...
if ($actesConventions->hasConvention($id)) {
    $html .= "<div class=\"col-md-6 alert alert-info\">
        <a href='" . \S2lowLegacy\Class\Helpers\UrlHelper::getLink("/admin/authorities/admin_authority_download_convention.php?authority_id=" . $id . "'>") .
...
if ($me->isSuper()) {
    $html .= "<br/><div><a class=\"btn btn-primary\" href='" . \S2lowLegacy\Class\Helpers\UrlHelper::getLink("/modules/actes/admin/actes_force_classifiction.php?authority_id=" . $id . "'>Envoyer une demande de classification</a></div>");
    $html .= "<br/><div><a class=\"btn btn-primary\" href='" . \S2lowLegacy\Class\Helpers\UrlHelper::getLink("/modules/actes/admin/actes_force_classifiction.php?force=1&authority_id=" . $id . "'>Envoyer demande de classification vide</a></div>");
}
if ($me->isGroupAdminOrSuper()) {
