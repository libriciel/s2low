...
if (! $me->authenticate()) {
    exitOrDisplayError($api, "Échec de l'authentification", \S2lowLegacy\Class\LegacyObjectsManager::getLegacyObjectInstancier()->get(\S2low\Helpers\RequeteHelper::class)->getLink("connexion-status"));
}
...
$him->setId($id);
if (! $him->init()) {
    exitOrDisplayError($api, "Erreur lors de la modification de l'utilisateur", \S2lowLegacy\Class\LegacyObjectsManager::getLegacyObjectInstancier()->get(\S2low\Helpers\RequeteHelper::class)->getLink("/admin/users/admin_users.php"));
} else {
    if (! $me->canEditUser($id)) {
        exitOrDisplayError($api, "Accès refusé pour la modification de cet utilisateur", \S2lowLegacy\Class\LegacyObjectsManager::getLegacyObjectInstancier()->get(\S2low\Helpers\RequeteHelper::class)->getLink("/admin/users/admin_users.php"));
    }
}
...
if ($userSQL->hasDoublon($him->getId(), $certificat_connexion_info, $user_info['login'], false)) {
    exitOrDisplayError($api, "Impossible de supprimer le certificat car l'opération entrainerait des doublons", \S2lowLegacy\Class\LegacyObjectsManager::getLegacyObjectInstancier()->get(\S2low\Helpers\RequeteHelper::class)->getLink("/admin/users/admin_user_edit.php?id={$him->getId()}"));
}
...
if ($api) {
    ...
    header("Location: " . \S2lowLegacy\Class\LegacyObjectsManager::getLegacyObjectInstancier()->get(\S2low\Helpers\RequeteHelper::class)->getLink("/admin/users/admin_user_edit.php?id=") . $him->getId());
}
