...
class MenuHTML
{
    ...
    public function getMenuContent()
    {
        ...
        <div class="well sidebar-nav">
                    <?php if ($this->userContext->userInfo) : ?>
                        <?php $this->displayUserMenu() ; ?>
                    <?php else : ?>
            <div id="menu-header">
    ...
    {
        $modulesInfo = $this->moduleSQL->getModulesForUser($this->userContext->userInfo);
        ...
        foreach ($modulesInfo as $i => $module) {
            ...
            if (file_exists(SITEROOT . "/public.ssl/modules/" . $module["name"] . "/admin/index.php")) {
                if (in_array($this->userContext->userInfo['role'], array('SADM','GADM'))) {
                    $module_admin[] = $module;
                }
            }
        ...
        $nb_days_before_certificate_expires = $this->getNbDaysBeforeCertificatExpire($this->userContext->userInfo); ?>


                    <div id="menu-header">
                        Bienvenue <?php hecho($this->userContext->userInfo['pretty_name']) ?><br />
                    ...
                    /** @var MessageAdminSQL $messageAdminSQL */
                        ...
                        ?>
                        Rôle <?php  echo $this->userContext->userInfo['role_str'] ?>
            <?php if ($this->userContext->userInfo['nb_user_with_my_certificate'] > 1) : ?>
            <br/><a href='<?php echo Helpers::getLink("logout.php")?>'>déconnexion</a>
            <?php endif;?>
                    </div>
                    <ul class="text-menu nav">
            <?php if (in_array($this->userContext->userInfo['role'], array('SADM','GADM','ADM'))) : ?>
            <li class="menu-list-title">Administration</li>
            <?php endif;?>
            
            <?php if ($this->userContext->userInfo['role'] == 'SADM') : ?>
            ...
            <?php endif;?>
            <?php if (in_array($this->userContext->userInfo['role'], array('SADM','GADM'))) : ?>
                <li><a href="<?php echo Helpers::getLink('/admin/authorities/admin_authorities.php')?>">Gestion des collectivités</a></li>
            <?php endif?>
            <?php if ($this->userContext->userInfo['role'] == 'ADM') : ?>
                <li><a href="<?php echo Helpers::getLink('/modules/mail/index.php?command=annuaire');?>">Carnet d'adresses de la collectivité</a></li>
                <li><a href="<?php echo Helpers::getLink("/admin/authorities/admin_authority_edit.php?id=" . $this->userContext->userInfo["authority_id"]); ?>">Paramètres collectivité</a></li>
            <?php endif;?>
            <?php if (! in_array($this->userContext->userInfo['role'], [User::USER,User::ARCH])) : ?>
            ...
            <?php if ($module_export && in_array($this->userContext->userInfo['role'], array('ADM','GADM','SADM'))) : ?>
            ...
            <li class="menu-list-title">Suivi <?php echo ! in_array($this->userContext->userInfo['role'], [User::USER,User::ARCH]) ? "du site" : ""?></li>
