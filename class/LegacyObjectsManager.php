...
class LegacyObjectsManager
{
    ...
    public static function setLegacyObjectInstancier(): void
    {
        ...
        if (isset($_SESSION)) {
            ...
            $environnement = new Environnement($_GET, $_POST, $_REQUEST, $_SESSION, $_SERVER, CONVERT_API_LOGINS_FROM_ISO);
        } else {
            $session = array();
            $objectInstancier->set(SessionWrapper::class, new SessionWrapper($session));
            $environnement = new Environnement($_GET, $_POST, $_REQUEST, $session, $_SERVER, CONVERT_API_LOGINS_FROM_ISO);
        }
